...
def run(variation, percentage=0.2) -> None:
    ...
    if variation.upper() == "LIBRARY":
        csv_lib = CsvURL(constants.FILES_PATH+constants.TEST_FILE, constants.LIBRARY, percentage)
    elif variation.upper() == "RAW":
        csv_lib = CsvURL(constants.FILES_PATH+constants.TEST_FILE, constants.RAW, percentage)
    else:
